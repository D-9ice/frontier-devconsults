  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      const payload = {
        ...
        requirements: cleanLines(form.requirements),
        screenshotUrls: cleanLines(form.screenshotUrls),
        technologies: cleanLines(form.technologies), responsibilities: cleanLines(form.responsibilities), challenges: cleanLines(form.challenges), deploymentOptions: cleanLines(form.deploymentOptions),
      };
      const response = await fetch(editingId ? `/api/admin/apps/${editingId}` : '/api/admin/apps', { method: editingId ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
...
function AppRow({ app, onEdit, onDelete }: { app: AppRecord; onEdit: () => void; onDelete: () => void }) { return <div className="flex flex-col gap-4 p-5 lg:flex-row lg:items-center lg:justify-between"><div className="flex min-w-0 items-center gap-4">{app.iconUrl ? <img src={app.iconUrl} alt="" className="h-12 w-12 rounded object-contain" /> : <div className="flex h-12 w-12 items-center justify-center rounded bg-blue-50 text-blue-600"><Smartphone className="h-6 w-6" /></div>}<div className="min-w-0"><p className="truncate font-bold text-gray-900">{app.name}</p><p className="text-sm text-gray-600">{app.category} · v{app.version} · order {app.sortOrder}</p></div></div><div className="flex flex-wrap items-center gap-3">{!app.iconUrl && <span className="rounded-full bg-amber-100 px-3 py-1 text-sm font-semibold text-amber-800">Artwork needed</span>}<span className={`inline-flex items-center gap-1 rounded-full px-3 py-1 text-sm font-semibold ${app.visibility === 'published' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>{app.visibility === 'published' ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}{app.visibility}</span><button onClick={onEdit} className="inline-flex h-10 w-10 items-center justify-center rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50" aria-label={`Edit ${app.name}`}><Edit3 className="h-4 w-4" /></button><button onClick={onDelete} className="inline-flex h-10 w-10 items-center justify-center rounded-lg border border-red-200 text-red-600 hover:bg-red-50" aria-label={`Delete ${app.name}`}><Trash2 className="h-4 w-4" /></button></div></div>; }
function AppEditor({ form, setForm, editing, saving, onClose, onSubmit }: { form: AppForm; setForm: (value: AppForm) => void; editing: boolean; saving: boolean; onClose: () => void; onSubmit: (event: FormEvent) => void }) {
  const update = <K extends keyof AppForm>(key: K, value: AppForm[K]) => setForm({ ...form, [key]: value });
  const addScreenshot = (url: string) => update('screenshotUrls', [...cleanLines(form.screenshotUrls), url]);
  const removeScreenshot = (url: string) => update('screenshotUrls', cleanLines(form.screenshotUrls).filter((item) => item !== url));

  return <section className="mb-8 rounded-lg bg-white p-6 shadow-sm">
    <div className="mb-6 flex items-center justify-between"><h2 className="text-xl font-bold text-gray-900">{editing ? 'Edit App' : 'New App'}</h2><button onClick={onClose} className="rounded p-2 text-gray-500 hover:bg-gray-100" aria-label="Close editor"><X className="h-5 w-5" /></button></div>
    <form onSubmit={onSubmit} className="space-y-5">
      ...
      <div className="grid gap-5 md:grid-cols-2"><TextArea label="Features (one per line)" value={form.features.join('\n')} onChange={(value) => update('features', preserveLines(value))} /><TextArea label="Requirements (one per line)" value={form.requirements.join('\n')} onChange={(value) => update('requirements', preserveLines(value))} /><TextArea label="Technologies (one per line)" value={form.technologies.join('\n')} onChange={(value) => update('technologies', preserveLines(value))} /><TextArea label="Responsibilities (one per line)" value={form.responsibilities.join('\n')} onChange={(value) => update('responsibilities', preserveLines(value))} /></div>
      <div className="grid gap-5 md:grid-cols-2"><TextArea label="Client problem (optional)" value={form.clientProblem || ''} onChange={(value) => update('clientProblem', value)} /><TextArea label="Solution summary (optional)" value={form.solutionSummary || ''} onChange={(value) => update('solutionSummary', value)} /></div>
      <div className="grid gap-5 md:grid-cols-2"><TextArea label="Engineering challenges (one per line)" value={form.challenges.join('\n')} onChange={(value) => update('challenges', preserveLines(value))} /><TextArea label="Deployment options (one per line)" value={form.deploymentOptions.join('\n')} onChange={(value) => update('deploymentOptions', preserveLines(value))} /><TextArea label="Support summary (optional)" value={form.supportSummary || ''} onChange={(value) => update('supportSummary', value)} /><TextArea label="Confidentiality note (optional)" value={form.confidentialityNote || ''} onChange={(value) => update('confidentialityNote', value)} /></div>
      <div className="grid gap-5 md:grid-cols-2"><Text label="App Icon URL" value={form.iconUrl || ''} onChange={(value) => update('iconUrl', value)} /><Text label="Video URL (optional)" value={form.videoUrl || ''} onChange={(value) => update('videoUrl', value)} /><MediaUpload label="App icon upload" bucket="app-media" kind="image" value={form.iconUrl || ''} onChange={(value) => update('iconUrl', value)} help="JPEG, PNG, WebP, or GIF up to 8 MB. Save the app after uploading." /><MediaUpload label="App video upload" bucket="app-media" kind="video" value={form.videoUrl || ''} onChange={(value) => update('videoUrl', value)} help="MP4 or WebM up to 50 MB. Save the app after uploading." /></div>
      <div className="grid gap-5 md:grid-cols-2"><TextArea label="Screenshot URLs (one per line)" value={form.screenshotUrls.join('\n')} onChange={(value) => update('screenshotUrls', preserveLines(value))} /><div className="space-y-3"><MediaUpload label="Add app screenshot" bucket="app-media" kind="image" value="" onChange={addScreenshot} showEmptyState={false} help="Uploads add another screenshot to the list. JPEG, PNG, WebP, or GIF up to 8 MB." />{cleanLines(form.screenshotUrls).length > 0 && <div className="grid grid-cols-3 gap-3">{cleanLines(form.screenshotUrls).map((url) => <div key={url} className="relative rounded-lg border border-gray-200 p-1"><img src={url} alt="App screenshot" className="h-20 w-full rounded object-cover" /><button type="button" onClick={() => removeScreenshot(url)} className="mt-1 w-full text-xs font-semibold text-red-600 hover:text-red-700">Remove from listing</button></div>)}</div>}</div></div>
      <div className="grid gap-5 md:grid-cols-2"><Text label="Play Store URL" value={form.playStoreLink || ''} onChange={(value) => update('playStoreLink', value)} /><Text label="Direct Download URL" value={form.downloadLink || ''} onChange={(value) => update('downloadLink', value)} /></div>
      <div className="grid gap-5 md:grid-cols-2"><Text label="Demo URL" value={form.demoUrl || ''} onChange={(value) => update('demoUrl', value)} /><Text label="License terms URL" value={form.licenseTermsUrl || ''} onChange={(value) => update('licenseTermsUrl', value)} /><NumberField label="Starting price (USD cents)" value={form.startingPriceUsdMinor} onChange={(value) => update('startingPriceUsdMinor', value)} /><Select label="Price display" value={form.priceVisibility} options={['enquire', 'from', 'show']} onChange={(value) => update('priceVisibility', value as AppForm['priceVisibility'])} /></div>
      <fieldset className="rounded-lg border border-gray-200 p-4"><legend className="px-2 text-sm font-semibold text-gray-700">Commercial options</legend><div className="flex flex-wrap gap-6"><Check label="Hosted license" checked={form.commercialModes.includes('hosted_license')} onChange={(value) => update('commercialModes', toggleValue(form.commercialModes, 'hosted_license', value))} /><Check label="White-label" checked={form.commercialModes.includes('white_label')} onChange={(value) => update('commercialModes', toggleValue(form.commercialModes, 'white_label', value))} /><Check label="Exclusive acquisition" checked={form.commercialModes.includes('exclusive_acquisition')} onChange={(value) => update('commercialModes', toggleValue(form.commercialModes, 'exclusive_acquisition', value))} /><Check label="Customization available" checked={form.customizationAvailable} onChange={(value) => update('customizationAvailable', value)} /></div></fieldset>
      <div className="flex justify-end gap-3"><button type="button" onClick={onClose} className="rounded-lg border border-gray-300 px-4 py-2 font-semibold text-gray-700 hover:bg-gray-50">Cancel</button><button disabled={saving} className="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 font-semibold text-white hover:bg-blue-700 disabled:opacity-60"><Save className="h-4 w-4" />{saving ? 'Saving...' : editing ? 'Save App' : 'Create App'}</button></div>
    </form>
  </section>;
}
...
function Check({ label, checked, onChange }: { label: string; checked: boolean; onChange: (value: boolean) => void }) { return <label className="inline-flex items-center gap-2 text-sm font-semibold text-gray-700"><input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} className="h-4 w-4" />{label}</label>; }
function toggleValue<T extends string>(items: T[], item: T, enabled: boolean) { return enabled ? Array.from(new Set([...items, item])) : items.filter((value) => value !== item); }
