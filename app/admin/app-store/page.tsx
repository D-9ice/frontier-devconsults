...
function AppEditor({ form, setForm, editing, saving, onClose, onSubmit }: { form: AppForm; setForm: (value: AppForm) => void; editing: boolean; saving: boolean; onClose: () => void; onSubmit: (event: FormEvent) => void }) {
  ...
  const addScreenshot = (url: string) => update('screenshotUrls', [...form.screenshotUrls, url]);
  const removeScreenshot = (url: string) => update('screenshotUrls', form.screenshotUrls.filter((item) => item !== url));
  ...
  return <section className="mb-8 rounded-lg bg-white p-6 shadow-sm">
    ...
    <form onSubmit={onSubmit} className="space-y-5">
      ...
      <div className="grid gap-5 md:grid-cols-2"><TextArea label="Screenshot URLs (one per line)" value={form.screenshotUrls.join('\n')} onChange={(value) => update('screenshotUrls', splitLines(value))} /><div className="space-y-3"><MediaUpload label="Add app screenshot" bucket="app-media" kind="image" value="" onChange={addScreenshot} showEmptyState={false} help="Uploads add another screenshot to the list. JPEG, PNG, WebP, or GIF up to 8 MB." />{form.screenshotUrls.length > 0 && <div className="grid grid-cols-3 gap-3">{form.screenshotUrls.map((url) => <div key={url} className="relative rounded-lg border border-gray-200 p-1"><img src={url} alt="App screenshot" className="h-20 w-full rounded object-cover" /><button type="button" onClick={() => removeScreenshot(url)} className="mt-1 w-full text-xs font-semibold text-red-600 hover:text-red-700">Remove from listing</button></div>)}</div>}</div></div>
